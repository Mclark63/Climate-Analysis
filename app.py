...
def tobs():
    ...
    # show date and temprature values
    tobs_list = []
    for result in results:
        r = {}
        r["date"] = result[1]
...
def start_end(start, end):
    ...
    # query data for the start date value
    start_date = session.query(func.min(Measurement.tobs), func.avg(Measurement.tobs), func.max(Measurement.tobs)).filter(Measurement.date >= start_dt).filter(Measurement.date <= end_dt)
